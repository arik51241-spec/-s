...
export default function Home() {
  ...
  useEffect(() => {
    let active = true;
    const sync = async () => {
      const response = await fetch("/api/site-state", { cache: "no-store" }).catch(() => null);
      if (!response?.ok || !active) return;
      const next = (await response.json()) as SiteState;
      setSiteState(next);
      setBoostUsed(localStorage.getItem(`birthday-boost-13-08-2026:${next.boostVersion}`) === "used");
    };
    sync();
    const poller = window.setInterval(sync, 5000);
  ...
  const takeBoost = () => {
    if (boostUsed || !siteState) return;
    localStorage.setItem(`birthday-boost-13-08-2026:${siteState.boostVersion}`, "used");
    setBoostUsed(true);
    setBoostOpen(true);
  };

  if (!siteState) {
    return <main className="stateLoading"><div><span>16</span><p>Загружаю волшебство…</p></div></main>;
  }

  if (siteState.countdownOnly) {
    return (
      ...
      <div className="shell">
        <header className="topbar">
          ...
          <nav aria-label="Навигация">
            <a href="#memories">моменты</a><a href="#wishes">желания</a><a href="#letter">письмо</a><a href="#boost">буст</a>
          </nav>
        ...
        <section className="hero" id="top">
          ...
          <div className="heroCopy">
            ...
            <div className="heroButtons"><a className="primary" href="#profile">Начать историю ↓</a><a className="secondary" href="#boost">Забрать буст</a></div>
          </div>
        ...
        <section className="section profile" id="profile">
          <div className="sectionHead"><span>01</span><p>КАРТОЧКА ГЛАВНОЙ ГЕРОИНИ</p></div>
        ...
        <section className="section chapters">
          <div className="sectionHead"><span>02</span><p>НАША ИСТОРИЯ В ЧЕТЫРЁХ ГЛАВАХ</p></div>
        ...
        <section className="section memories" id="memories">
          <div className="sectionHead"><span>03</span><p>СОЗВЕЗДИЕ НАШИХ МОМЕНТОВ</p></div>
        ...
        <section className="section ocPassport">
          <div className="sectionHead"><span>04</span><p>ПАСПОРТ ТВОЕГО ОС</p></div>
        ...
        <section className="section compatibility">
          <div className="sectionHead"><span>05</span><p>РЕЗУЛЬТАТ СКАНИРОВАНИЯ</p></div>
        ...
        <section className="section wishes" id="wishes">
          <div className="sectionHead"><span>06</span><p>16 КРИСТАЛЛОВ ЖЕЛАНИЙ</p></div>
        ...
        <section className="section letter" id="letter">
          <div className="sectionHead"><span>07</span><p>ТО, ЧТО Я ХОЧУ ТЕБЕ СКАЗАТЬ</p></div>
        ...
        <section className="section promises">
          <div className="sectionHead"><span>08</span><p>ШЕСТЬ ОБЕЩАНИЙ НА СЛЕДУЮЩУЮ ГЛАВУ</p></div>
        ...
        <section className="section coupons">
          <div className="sectionHead"><span>09</span><p>ПОДАРКИ, КОТОРЫЕ НЕ ЗАКАНЧИВАЮТСЯ</p></div>
